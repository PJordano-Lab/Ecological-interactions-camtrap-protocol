...
This code will create the database from cameratraps fieldwork after eliminating empty videos with MD and revise them in Timelapse for species identification, behaviour and annotations. 

Read the resulting .csv file from Time-lapse visualization (after running the AI process).
```{r}
library(stringr)
library(dplyr)
library(lubridate)
...
CREATE PLANT NAME
Here we create a new column with the plant name. We also subset the dataset to non empty videos, eliminate car and person. Also correct mistaken names. 
